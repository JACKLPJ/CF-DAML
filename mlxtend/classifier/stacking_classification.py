...
#                 all_task = [executor.submit(url.fit, (X,y)) 
#                             for url in self.clfs_]
#             else: 
#                 all_task = [executor.submit(url.fit, (X,y,sample_weight)) 
#                             for url in self.clfs_]
#             all_results = []
...
#                 all_results.append(future.result())
            ...
            for clf in self.clfs_: 
                if sample_weight is None:
                    if self.preprocessing:
                        all_results.append(
                                       pool.apply_async(clf[0].fit,
                                                        args=(
                                                            sm.Preprocessing(X, clf[1]),y,
                                                        )))
                    else:
                        all_results.append(
                                       pool.apply_async(clf[0].fit,
                    ...
                    if self.preprocessing:
                        all_results.append(
                                   pool.apply_async(clf[0].fit,
                                                    args=(
                                                        sm.Preprocessing(X, clf[1]),y,sample_weight,
                                                    )))
                    else:
                        all_results.append(
                                   pool.apply_async(clf[0].fit,
            ...
            for a in range(len(self.clfs_)):
                try:
                    self.clfs_[a][0]=all_results[a].get()
                except:
...
#                    clf.fit(X, y, sample_weight=sample_weight)
    ...
    def predict_meta_features(self, X):
        ...
        if self.use_probas:
#             executor = ThreadPoolExecutor()
#             all_task = [executor.submit(url.predict_proba, (X)) 
#                             for url in self.clfs_]
...
#             all_results = []
            ...
            for clf in self.clfs_: 
                if self.preprocessing:
                    try:
                        all_results.append(
                           pool.apply_async(clf[0].predict_proba,
                                            args=(
                                                sm.Preprocessing(X, clf[1]),
                                            )))
                    except:
                        continue
...
#                 for future in as_completed(all_task):
#                     try:
#                         all_results.append(future.result()[:, :-1])
#                     except:
#                         continue
                for sub_res in all_results:
                    try:
                        All_res.append(sub_res.get()[:, :-1])
                    except:
                        continue
#                 all_results = [
#                    sub_res.get()[:, :-1] for sub_res in all_results
#                 ]
                probas = np.asarray(All_res)
                #probas = np.asarray(all_results)
...
#             executor = ThreadPoolExecutor()
#             all_task = [executor.submit(url.predict, (X)) 
#                             for url in self.clfs_]
...
#                     continue
            ...
            for clf in self.clfs_: 
                if self.preprocessing:
                    #try:
                    all_results.append(
                       pool.apply_async(clf[0].predict,
                                        args=(
                                            sm.Preprocessing(X, clf[1]),
                                        )))
#                     except:
