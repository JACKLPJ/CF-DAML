...
class data_modeling:
    def __init__(self, X, y, k, N, add_dff, add_pipeline, add_dT50, verbose):
        #GM.add_dff=add_dff
        ...
        self.MODEL, self.lda_n = GM.get_model(X, y, k, N, add_dff,
                                              add_pipeline, add_dT50, verbose).Model
        ...
        if self.verbose:
            print('#######################################')
            for i in range(n):
                print('The data pre_processing method is:  {};'.format(
                self.result_preprocessor[i]))
    ...
    def pre_processing(self, dic_config):
        ...
        if processing_method == 'polynomial':
            ...
            paras['interaction_only'] = True if dic_config[
                'polynomial_interaction_only'] else False
            paras['include_bias'] = True if dic_config[
                'polynomial_include_bias'] else False
#         else:
    ...
    def model(self, dic_config):
        ...
        if model_ == 'xgradient_boosting':
            paras['colsample_bytree'] = dic_config[
                'xgradient_boosting_colsample_bytree']
            paras['colsample_bylevel'] = dic_config[
                'xgradient_boosting_colsample_bylevel']
            paras['alpha'] = dic_config['xgradient_boosting_reg_alpha']
            # paras['scale_pos_weight']=dic_config['xgradient_boosting_scale_pos_weight']
            paras['learning_rate'] = dic_config[
                'xgradient_boosting_learning_rate']
            paras['max_delta_step'] = dic_config[
                'xgradient_boosting_max_delta_step']
            paras['base_score'] = dic_config['xgradient_boosting_base_score']
            paras['n_estimators'] = dic_config[
                'xgradient_boosting_n_estimators']
            paras['subsample'] = dic_config['xgradient_boosting_subsample']
            paras['reg_lambda'] = dic_config['xgradient_boosting_reg_lambda']
            paras['min_child_weight'] = dic_config[
                'xgradient_boosting_min_child_weight']
            ...
            paras['n_components'] = min(int(dic_config['lda_n_components']),
                                        int(self.lda_n))
            paras['tol'] = dic_config['lda_tol']
        elif model_ == 'extra_trees':
            paras['bootstrap'] = True if dic_config[
                'extra_trees_bootstrap'] else False
            ...
            paras['min_weight_fraction_leaf'] = dic_config[
                'extra_trees_min_weight_fraction_leaf']
            ...
            paras['bootstrap'] = True if dic_config[
                'random_forest_bootstrap'] else False
            ...
            paras['min_weight_fraction_leaf'] = dic_config[
                'decision_tree_min_weight_fraction_leaf']
            ...
            paras['n_neighbors'] = dic_config[
                'k_nearest_neighbors_n_neighbors']
        elif model_ == 'bernoulli_nb':
            paras['alpha'] = dic_config['bernoulli_nb_alpha']
            paras['fit_prior'] = True if dic_config[
                'bernoulli_nb_fit_prior'] else False
        elif model_ == 'multinomial_nb':
            paras['alpha'] = dic_config['multinomial_nb_alpha']
            paras['fit_prior'] = True if dic_config[
                'multinomial_nb_fit_prior'] else False
        #else:
        elif model_ == 'qda':
